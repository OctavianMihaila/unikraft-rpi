...
void print_dtb_header(void *dtb) {
    ...
    uk_pr_err("\n--- Device Tree Header ---\n");
    uk_pr_err("Magic: 0x%x\n", magic);
    uk_pr_err("Total Size: 0x%x\n", totalsize);
    uk_pr_err("DT Structure Offset: 0x%x\n", off_dt_struct);
    uk_pr_err("DT Strings Offset: 0x%x\n", off_dt_strings);
}
